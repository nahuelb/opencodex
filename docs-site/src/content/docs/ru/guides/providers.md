...
**Muse Spark в Zen использует `/zen/v1/responses`.** Это относится к версиям 1.3 и 1.2, включая `-contributor-free`. Пресеты `opencode-zen` и `opencode-free` представляются как `opencodex` и формируют непрозрачный заголовок `x-opencode-session` из идентификатора разговора. Значение сохраняется между ходами и различается для отдельных разговоров. Явно заданный заголовок провайдера имеет приоритет.

Клиент должен передавать `thread-id`, `session-id`, `x-opencode-session` или `metadata.user_id` для Claude Code. Без идентификатора Zen может вернуть `MissingSessionID`. Добавление одного API-ключа не устраняет эту ошибку. Бесплатная Muse Spark 1.3 успешно ответила при проверке с ключом Zen; доступ без ключа и доступность моделей определяет [OpenCode Zen](https://opencode.ai/docs/zen/).
