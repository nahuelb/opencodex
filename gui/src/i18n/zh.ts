...
export const zh: Record<TKey, string> = {
  ...
  "logs.cache.hit": "命中",
  "logs.cache.miss": "未命中",
  "logs.cache.unknown": "未知",
  "logs.cache.summary": "缓存使用量",
  "logs.cache.hitRate": "请求命中率",
  "logs.cache.reuseRate": "输入复用率",
  "logs.cache.scope": "仅统计已加载且筛选后的请求。比率排除未知用量；输入复用率按输入令牌数加权。",
  "logs.cache.definition": "报告的缓存读取大于零表示命中，明确报告为零表示未命中。缺失、估算或无效的用量为未知。",
  "logs.cache.readTokens": "{tokens} 已缓存",
  "logs.cache.outcome": "提供商结果",
  "logs.cache.uncached": "未复用的输入",
  "logs.cache.writeNote": "未复用的输入包含缓存写入。写入不代表命中。",
  "logs.cache.astra": "Astra 保留",
  "logs.cache.sideChat": "侧边聊天前缀复用",
  "logs.cache.matchedItems": "匹配的输入项",
  "logs.cache.attempt": "尝试",
  "logs.cache.local": "本地缓存诊断",
  "logs.cache.localNote": "这些本地决策不能证明提供商缓存命中。尝试数据单独显示，不计入请求总量。",
...
"common.discard": "丢弃",
  ...
  "dash.visionOff": "关闭",
  "manualCompact.title": "手动压缩",
  "manualCompact.description": "选择手动 /compact 命令使用的模型。自动压缩和后续消息保留对话设置。",
  "manualCompact.model": "压缩模型",
  "manualCompact.effort": "推理强度",
  ...
  "manualCompact.comboWarning": "启用此设置后，每次手动 /compact 都会将完整对话内容发送给组合 {combo} 的每个目标（{providers}），包括故障转移目标，以进行摘要。",
  "manualCompact.comboProvidersUnknown": "其已配置的目标提供商",
