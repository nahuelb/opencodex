...
mechanism above with a separate Zen hash domain. Client session headers and Claude metadata
supply identity; shared prompt cohorts never substitute for a conversation. Zen omits the
header without a real conversation, while Go falls back to the request-scoped lane it needs
on every request.
