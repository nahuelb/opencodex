...
export async function startCacheProxy(nativeWebSocket = false, enabled = true) {
  const home = mkdtempSync(join(tmpdir(), "side-cache-proxy-"));
  ...
  function events(body: any, transport: "http" | "websocket", compact = false) {
    ...
    return [
      ...
      { type: "response.completed", response: { id, model: body.model, object: "response", status: "completed", output, usage: { input_tokens: 100, output_tokens: 1, input_tokens_details: { cached_tokens: 80 } } } },
    ];
  }
  ...
  globalThis.fetch = ((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    return Promise.reject(new Error("Synthetic cache fixture denies external fetch"));
  }) as typeof fetch;
  globalThis.WebSocket = new Proxy(RealWebSocket, {
    construct(target, args) {
      ...
      if (url.hostname !== "127.0.0.1") throw new Error("Synthetic cache fixture denies external websocket");
  ...
  try {
    const [{ saveConfig }, { startServer }, { prepareSideChatCache }] = await Promise.all([
      import("../../src/config"), import("../../src/server"), import("../../src/codex/side-chat-cache")]);
    ...
    saveConfig({ port: 0, hostname: "127.0.0.1", defaultProvider: "openai", openaiProviderTierVersion: 2, websockets: true,
      providers: { openai: { adapter: "openai-responses", baseUrl: "https://chatgpt.com/backend-api/codex", authMode: "forward", codexAccountMode: "direct", experimentalCodexSideChatCache: enabled } } });
    proxy = startServer(0);
  } catch (error) { await stop(); throw error; }
  const url = new URL("/v1/responses", proxy!.url);
  function headers(thread: string, parent?: string) {
    return { "content-type": "application/json", "x-opencodex-api-key": "fixture-admission", authorization: `Bearer ${fakeChatGptJwt({ chatgpt_account_id: "fixture-account" })}`, "chatgpt-account-id": "fixture-account", "thread-id": thread, "session-id": thread, "x-codex-parent-thread-id": parent ?? thread };
  }
  async function http(body: unknown, thread: string, compact = false, parent?: string) {
    const response = await realFetch(compact ? `${url}/compact` : url, { method: "POST", headers: headers(thread, parent), body: JSON.stringify(body), signal: AbortSignal.timeout(10_000) });
  ...
  function websocket(thread: string, parent?: string) {
    const wsUrl = new URL(url); wsUrl.protocol = "ws:";
    const ws = new RealWebSocket(wsUrl, { headers: headers(thread, parent) } as unknown as string[]);
