...
for (const name of ["view_image", "default.view_image"]) {
  test(`${name} is restored across native Responses JSON and SSE`, () => {
    const args = JSON.stringify({ path: "/tmp/chart.png", detail: "original" });
    const item = { type: "function_call", id: "fc_image", call_id: "call_image", name, arguments: args, status: "completed" };
    const names = new Set(["exec"]);
    const restored = JSON.parse(restoreRoutedCustomCallsInJson(JSON.stringify({ output: [item] }), names, new Set(), names));
    expect(restored.output[0]).toMatchObject({ type: "custom_tool_call", name: "exec", call_id: "call_image" });
    expect(restored.output[0].input).toContain("await tools.view_image(");
    expect(restored.output[0].input).toContain("image(result.image_url, result.detail)");
    const rewrite = createRoutedCustomToolRestoreBlockRewrite(names, undefined, new Set(), names);
    const added = rewrite(frame("response.output_item.added", { output_index: 0, item: { ...item, arguments: "", status: "in_progress" } }));
    expect(dataPayload(added[0]!).item).toMatchObject({ name: "exec", type: "custom_tool_call" });
    expect(rewrite(frame("response.function_call_arguments.delta", { item_id: item.id, delta: args.slice(0, 12) }))).toEqual([]);
    expect(rewrite(frame("response.function_call_arguments.delta", { item_id: item.id, delta: args.slice(12) }))).toEqual([]);
    const done = rewrite(frame("response.function_call_arguments.done", { item_id: item.id, arguments: args }));
    expect(done.map(dataPayload).some(value => typeof value.input === "string" && value.input.includes("await tools.view_image("))).toBe(true);
    const terminal = rewrite(frame("response.completed", { response: { output: [item] } }));
    expect(JSON.stringify(terminal)).not.toContain('default.view_image');
    expect(terminal.join("")).toContain("image(result.image_url, result.detail)");
  });
}

  test("handleResponses repairs default.view_image through the complete relay", async () => {
    const savedFetch = globalThis.fetch;
    let outboundBody: Record<string, unknown> | undefined;
    const upstreamItem = {
      type: "function_call",
      ...
      arguments: JSON.stringify({ path: "/tmp/chart.png", detail: "original" }),
      status: "completed",
    };
    const upstream = [
      frame("response.output_item.added", { output_index: 0, item: { ...upstreamItem, arguments: "", status: "in_progress" } }),
    ...
    try {
      ...
      expect(clientSse).toContain('"type":"custom_tool_call"');
      expect(clientSse).toContain('"type":"response.custom_tool_call_input.done"');
      expect(clientSse).toContain("await tools.view_image(");
      expect(clientSse).toContain("image(result.image_url, result.detail)");
      expect(clientSse).not.toContain("response.failed");
      expect(clientSse).not.toContain("response.function_call_arguments.done");
      expect(clientSse).not.toContain('"type":"function_call"');
      expect(clientSse).toContain("data: [DONE]");
    } finally {
